...
class HiddenStateGenerateRandom(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.min_ctx = random.uniform(0.2, 0.6)
    ...
    def step(self, action):
        ...
        if action >= 1:
            ...
            try:
                ...
                self.done = True  # no more input_ids, sample "finished"
                return None, 0, True, False, {}
            if optimal_exit_layer_for_token <= 0 and not self.done:
                try:
    ...
    def compute_reward(self, state, action, label, first_last_match, last_layer_pred, optimal_exit_layer_for_token):
        ...
        if action == 1:  # Exit
            if state['prediction'] == label and self.is_best_exit(optimal_exit_layer_for_token,
                ...
                reward = max_reward * beta
            elif state['prediction'] == label:
                reward -= (curr_layer_norm - optimal_exit_layer_norm) * (self.num_layers - 1) * delta
            ...
            if curr_layer_norm < optimal_exit_layer_norm:
                ...
                reward -= self.allowed_exits[-1] * gamma
            else:
                reward -= (self.get_next_layer_norm(curr_layer_norm) - optimal_exit_layer_norm) * (
                            self.num_layers - 1) * delta

        if reward >= 0:
            pass
        else:
            reward = self.min_max_scaling_to_range(reward, -(self.allowed_exits[-1] * gamma), 0, -1, 0)
