...
class HiddenStateGenerateRandomAlignment(gym.Env):
    def __init__(self, model_id, num_layers, device, dataset, debug=True, start_sample=0,
                 ...
                 max_samples=None):
        super(HiddenStateGenerateRandomAlignment, self).__init__()
        random.seed(42)
    ...
    def compute_reward(self, state, action, first_last_match, last_layer_pred):
        ...
        layer = curr_layer_norm * (self.num_layers - 1)
        try:
            index = self.allowed_exits.index(layer)  # Find the index of `layer` in the list.
        except ValueError:
            index = 0
        position = index / (len(self.allowed_exits) - 1)  # Normalize to a value between 0 and 1.
