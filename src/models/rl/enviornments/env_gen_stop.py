...
class HiddenStateGenerateRandom(gym.Env):
    def __init__(self, model_id, num_layers, device, dataset, debug=True, start_sample=0,
                 allowed_exits: List[int] = None, model_max_ctx: int = 1024, cache_dir=None, min_ctx=0.3, max_new=15,
                 max_samples=None):
        super(HiddenStateGenerateRandom, self).__init__()
        random.seed(42)
        ...
        # Action space: 0 (continue), 1 (exit)
    ...
    def reset(self, seed=None, options=None):
        ...
        logger.info(
            f"Reset, last episode steps:{self.last_episode_steps}, reward:{self.last_episode_reward}, mean rew/step:{self.last_episode_reward / self.last_episode_steps}")
        self.last_episode_reward = 0
        self.last_episode_steps = 0
        is_ok_sample = False
        while not is_ok_sample:
            ...
            label = self.current_input_ids[:, self.current_ctx + self.current_token]
            current_optimal, first_last_match, last_layer_token = self.get_optimal_exit_point(label)

            if current_optimal <= 0:
                while current_optimal <= 0:
                    if (self.current_token + 1) >= self.states_length or self.current_ctx < self.current_token:
                        self.current_sample = random.randint(0, len(self.dataset) - 1)
    ...
    def step(self, action):
        ...
        try:
            label = self.current_input_ids[:, self.current_ctx + self.current_token]
        except IndexError:  # sample ended
            return None, 0, True, False, {}
        optimal_exit_layer_for_token, first_last_match, last_layer_token = self.get_optimal_exit_point(label)

        # print("optimal: ", {optimal_exit_layer_for_token}, "curr layer: ", self.layer_state, "action:" ,action, " curr-tks: ", self.current_token)
        ...
        logger.info(
            f"optimal layer:  {optimal_exit_layer_for_token}  current layer: {self.layer_state}  reward:  {reward}  action:  {action}")
        if self.DEBUG:
            print(
        ...
        if action >= 1:
            ...
            label = self.current_input_ids[:, self.current_ctx + self.current_token]
            optimal_exit_layer_for_token, first_last_match, last_layer_token = self.get_optimal_exit_point(label)
            if optimal_exit_layer_for_token <= 0:
                self.get_next_feasible()
            self.current_step += 1
        else:  # continue
            ...
            if self.layer_state == self.allowed_exits[-1] and (self.current_token + 1) < self.states_length:
                ...
                self.layer_state = 0
                self.current_token += 1  # no exit, but "completed" current token
                self.current_step += 1
                self.llm_states_curr_tk = self.model_states[self.current_token]
            elif self.layer_state == self.allowed_exits[-1]:  # if no more token "left", but completed current token
                self.current_step += 1
            else:
    ...
    def compute_reward(self, state, action, label, first_last_match, last_layer_pred, optimal_exit_layer_for_token):
        ...
        # Reward for exiting
        if action == 1:  # Exit
            if state['prediction'] == label and self.is_best_exit(optimal_exit_layer_for_token,
                                                                  curr_layer_norm * (self.num_layers - 1)):
                reward = max_reward
            elif state['prediction'] == label:
                reward -= (curr_layer_norm - optimal_exit_layer_norm) * (self.num_layers - 1) * alpha
            else:  # exit without correct prediction
                if curr_layer_norm < optimal_exit_layer_norm:
                    penalty = (optimal_exit_layer_norm - curr_layer_norm) * (self.num_layers - 1)

                    reward -= penalty * beta
                else:
                    reward -= max_reward  # unlikely but possible case, will be scaled down (see scaling between -1 and 1)
            ...
            if curr_layer_norm < optimal_exit_layer_norm:
                ...
                reward -= (self.get_next_layer_norm(curr_layer_norm) - optimal_exit_layer_norm) * (
                            self.num_layers - 1) * gamma

        if reward >= 0:
            scaled_rew = self.min_max_scaling_to_range(reward, 0, max_reward, 0, 1)
        else:
            scaled_rew = self.min_max_scaling_to_range(reward, -(self.allowed_exits[-1] * gamma), 0, -1, 0)

        return scaled_rew.item() if isinstance(scaled_rew, torch.Tensor) else scaled_rew
