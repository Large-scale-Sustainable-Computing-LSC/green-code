...
class OPTDecoderWithExit(OPTDecoder):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.mode == "infer_ee" and idx in self.exit_positions:
...
class OPTModelWithExit(OPTModel):
    ...
    def __init__(self, config: OPTConfig, exit_indices: List[int], mode: str, rl_model: SelfBaseAlgorithm,
        ...
        self.decoder = OPTDecoderWithExit(config, exit_indices, mode, rl_model, temprature=temp)
...
class OPTEESingleHeadRLHiddenState(OPTForCausalLM):

    def __init__(self, config: OPTConfig, threshold: float = None, mode='infer', exit_indices: List[int] = None,
                 rl_model: SelfBaseAlgorithm = None, temp: float = 1.0, measure_rl_energy: bool = False):
        super().__init__(config)
        ...
        self.exit_indices = exit_indices
        self.model = OPTModelWithExit(config, exit_indices, mode, rl_model, temp=temp)
